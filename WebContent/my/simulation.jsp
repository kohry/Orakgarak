...
<div style="float:right; margin-right: 20px; margin-bottom: 15px; font-weight: bold">현재 남은 현금 : \ <fmt:formatNumber>${requestScope.simulation.remainCash }</fmt:formatNumber></div><br/><br/>
<div style="float:right; margin-right: 20px; margin-bottom: 15px; font-weight: bold">합계총액 : \ <fmt:formatNumber>${requestScope.simulation.remainCash+requestScope.simulation.evaluatedAsset }</fmt:formatNumber></div>
	...
	<table class="ui sortable table segment simulation">
		...
		<div id="rankinfo">
			<h3>투자랭킹</h3>
			<span>나의 투자 순위 : ${myRank.rank }위</span>
			<table class="ui table segment simulation">
				<thead>
					<tr>
						<th>순위</th>
						<th>ID</th>
				...
				<tbody>
					<c:forEach items="${topRank}" var="top">
						<tr>
							<td><fmt:formatNumber>${top.rank}</fmt:formatNumber></td>
							<td>${top.userId}</td>
							<td><fmt:formatNumber>${top.totalAsset}</fmt:formatNumber></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
