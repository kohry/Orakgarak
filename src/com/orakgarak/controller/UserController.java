...
import com.orakgarak.util.CommonUtil;
import com.orakgarak.util.SendMail;
...
public class UserController {
	...
	public String studentRegist(User user, Model model, HttpServletRequest request) throws NoSuchAlgorithmException {
		
		user.setPassword(CommonUtil.toSHA256(user.getPassword()));
	...
	public String myPage(Model model,HttpSession session){
		...
		String userId = user.getEmail(); //email이 유저아이디니 이렇게 합니다.
	...
	public String simulationPage(Model model,HttpSession session){
		...
		model.addAttribute("topRank",userService.findTopRank());
		model.addAttribute("myRank",userService.findMyRank(userId));
	...
	public HashMap<String, Object> simulation(HttpSession session){
		...
		if (userId != null) {
			...
			map.put("myRank", userService.findMyRank(userId).getRank());
	...
	public String welcome(Model model){
		model.addAttribute("contentPage", "welcome.jsp");
		return "index";
	}
