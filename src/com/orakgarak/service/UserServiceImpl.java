...
import com.orakgarak.vo.SimulationRank;
...
public class UserServiceImpl implements UserService {
	...
	public boolean create(User user) {
		...
		user.setAuthkey(authkey.toString().replace("-",""));
	...
	public UserAdvanced updateAndSelectSimulationWithCurrentPrice(String userId){
		...
		for(SimulatedStock s : list){
			...
			//그리고 현재 simulatedstock의 가격으로 세팅해준다.
			s.setCurrentPrice(price);
			if(price == 0){
				return null;
			}
			//업데이트된 가격하나의 최종프라이스가 답이 되고,
			totalPrice += s.getOldVolume() * price;
			
		}
		

		userAdvanced.setEvaluatedAsset(totalPrice); //살때를 기준으로 항상 계싼을 해야함
	...
	@Override
	public List<SimulationRank> findTopRank() {
		return userDao.findTopRank();
	}

	@Override
	public SimulationRank findMyRank(String userId) {
		return userDao.findMyRank(userId);
	}
